...
function runSyncOrAsync(fn, context, args, callback) {
	...
	var innerCallback = context.callback = function() {
		log("runSyncOrAsync -> async result", arguments);
		if(isDone) {
			if(reportedError) return; // ignore
	...
	try {
		...
		var result = (function LOADER_EXECUTION() {
			log("runSyncOrAsync -> loader", fn);
			return fn.apply(context, args);
		}());
		log("runSyncOrAsync -> sync result", result);
		// 同步loader 的结果处理逻辑
		if(isSync) {
			isDone = true;
			if(result === undefined)
...
function iteratePitchingLoaders(options, loaderContext, callback) {
	...
	if(loaderContext.loaderIndex >= loaderContext.loaders.length) {
		log("iteratePitchingLoaders -> processResource 最后一个 loader");
	...
	if(currentLoaderObject.pitchExecuted) {
		log("iteratePitchingLoaders -> pitchExecuted, next");
	...
	loadLoader(currentLoaderObject, function(err) {
		...
		// 如果没有 pitch 则递归 iteratePitchingLoaders, 这样当前loader pitch 相当于执行过了
		if(!fn) {
			log("iteratePitchingLoaders -> no pitch");
			return iteratePitchingLoaders(options, loaderContext, callback);
		}
...
function iterateNormalLoaders(options, loaderContext, args, callback) {
	...
	if(loaderContext.loaderIndex < 0) {
		log("iterateNormalLoaders -> <0 没有loader");
	...
	if(currentLoaderObject.normalExecuted) {
		log("iterateNormalLoaders -> normalExecuted, prev");
		loaderContext.loaderIndex--;
	...
	if(!fn) {
		log("iterateNormalLoaders -> no normal");
		return iterateNormalLoaders(options, loaderContext, args, callback);
	}
